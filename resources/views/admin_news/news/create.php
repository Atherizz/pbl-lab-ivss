...
// Pastikan $old_data didefinisikan untuk menghindari error PHP notice
            ...
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form action="<?= BASE_URL ?? '.' ?>/admin-berita/news" method="POST" enctype="multipart/form-data">
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div id="drop-area" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md cursor-pointer transition duration-150 ease-in-out hover:border-blue-500">
                                <div class="space-y-1 text-center">
                                    ...
                                    <img id="image-preview" class="max-h-32 w-auto mx-auto rounded-md object-cover mb-2 hidden" alt="Image Preview">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="image_file_input" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                            ...
                                            <!-- Input file diberi ID baru untuk digunakan di JS, dan disembunyikan secara visual (sr-only) -->
                        ...
                        <!-- Pastikan Anda mengambil user ID yang benar dari sesi di sini -->
...
<script>
    // --- Logika Drag and Drop JavaScript ---
    ...
    // 1. Mencegah perilaku default browser (membuka gambar) untuk event drag/drop
    ...
    // 2. Highlighting area saat file di atasnya (UX)
    dropArea.addEventListener('dragenter', () => {
        dropArea.classList.add('border-blue-500');
        dropArea.classList.add('bg-blue-50'); // Tailwind class
    }, false);
    ...
    // 4. Mengizinkan klik pada area drop untuk membuka dialog file
    dropArea.addEventListener('click', (e) => {
        // Hanya picu input jika yang diklik BUKAN label "Upload a file" (untuk menghindari double click)
        if (!e.target.closest('label')) {
            fileInput.click();
        }
    });

    // 5. Menangani file dari dialog atau drop
    ...
    function handleFiles(files) {
        ...
        const resetUI = (isError = false, errorMessage = '') => {
            ...
            fileNameDisplay.textContent = isError ? errorMessage : 'or drag and drop';
            
            // Kosongkan input file jika ada error
            if (isError) {
        ...
        if (!file) {
            resetUI(false, 'or drag and drop');
            return;
        }

        // Cek tipe file sederhana di sisi klien
        if (file.type.match('image.*')) {
            ...
            // Logika Preview Gambar menggunakan FileReader
            const reader = new FileReader();
            reader.onload = function(e) {
                imagePreview.src = e.target.result;
                imagePreview.classList.remove('hidden'); // Tampilkan preview
                imageIcon.classList.add('hidden'); // Sembunyikan ikon SVG
            };
