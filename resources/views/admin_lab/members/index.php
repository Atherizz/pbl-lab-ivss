<?php 
$pageTitle = 'Manajemen Anggota Lab';
$activeMenu = 'members'; 
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold text-gray-700">Kelola Anggota Lab</h2>
                    <div class="flex items-center gap-4">
                        <?php 
                        $userCount = $totalItems ?? 0;
                        ?>
                        <p class="text-sm text-gray-500 mt-1">Total: <?= $userCount ?> anggota</p>
                        <a href="<?= BASE_URL ?? '.' ?>/admin-lab/members/create"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php if (!empty($users)) : ?>
                                <?php foreach($users as $row) : ?>
                                    <?php
                                    $roleText = htmlspecialchars($row['role'] ?? 'unknown');
                                    $roleClass = match ($row['role']) {
                                        'admin_lab'   => 'bg-purple-600 text-white font-bold',
                                        'anggota_lab' => 'bg-cyan-600 text-white font-bold',
                                        'admin_berita' => 'bg-indigo-600 text-white font-bold',
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                        <?= htmlspecialchars($row['reg_number']) ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?= $roleClass ?>">
                                            <?= ucwords(str_replace('_', ' ', $roleText)) ?>
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <form action="<?= (BASE_URL ?? '.') . '/admin-lab/members/' . $row['id'] . '/delete' ?>" method="POST" class="inline">
                                            <input type="hidden" name="_method" value="DELETE">
                                            <button type="submit" 
                                                            class="text-red-600 hover:text-red-900" 
                                                            title="Hapus"
                                                            onclick="return confirm('Apakah Anda yakin ingin menghapus anggota <?= htmlspecialchars($row['name'], ENT_QUOTES) ?>?');">
                            ...
                            <?php else : ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-8 whitespace-nowrap text-sm text-gray-500 text-center">
                                        ...
                                        <p class="text-gray-600 font-medium">Tidak ada anggota ditemukan.</p>
                                        <p class="text-xs text-gray-400 mt-1">Klik "Tambah Anggota" untuk menambahkan pengguna baru.</p>
                                    </td>
                                </tr> 
                            <?php endif; ?>
                        </tbody>
                        ...
                        <nav class="flex justify-between items-center text-sm text-gray-500">
                            ...
                            <div class="flex space-x-1">
                                <?php if ($currentPage > 1) : ?>
                                    <a href="<?= BASE_URL ?? '.' ?>/admin-lab/members?page=<?= $currentPage - 1 ?>" 
                                ...
                                <div class="flex space-x-1">
                                    ...
                                    if ($startPage > 1) : ?>
                                        <a href="<?= BASE_URL ?? '.' ?>/admin-lab/members?page=1" 
                                           class="px-3 py-2 border rounded-md hover:bg-gray-100 transition">
                                            1
                                        </a>
                                        <?php if ($startPage > 2) : ?>
                                    ...
                                    <?php for ($i = $startPage; $i <= $endPage; $i++) : ?>
                                        ...
                                        <?php else : ?>
                                            <a href="<?= BASE_URL ?? '.' ?>/admin-lab/members?page=<?= $i ?>" 
                                               class="px-3 py-2 border rounded-md hover:bg-gray-100 transition">
                                                <?= $i ?>
                                            </a>
                                        <?php endif; ?>
                                    ...
                                    if ($endPage < $totalPages) : ?>
                                        <?php if ($endPage < $totalPages - 1) : ?>
                                            <span class="px-3 py-2">...</span>
                                        <?php endif; ?>
                                        <a href="<?= BASE_URL ?? '.' ?>/admin-lab/members?page=<?= $totalPages ?>" 
                                           class="px-3 py-2 border rounded-md hover:bg-gray-100 transition">
                                            <?= $totalPages ?>
                                        </a>
                                    <?php endif; ?>
