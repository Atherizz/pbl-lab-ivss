<?php 
$pageTitle = 'Edit Produk Lab';
$activeMenu = 'products';

$product = $product ?? []; 

$old_data = $old_data ?? $product;
$errors = $errors ?? [];
$errorMessage = flash('error') ?? null;

$produkTypeData = $old_data['produk_type'] ?? '[]';
$featuresData = $old_data['features'] ?? '[]';

if (is_string($produkTypeData)) {
    $produkTypeData = json_decode($produkTypeData, true) ?? [];
}
if (is_string($featuresData)) {
    $featuresData = json_decode($featuresData, true) ?? [];
}

$jsProdukType = json_encode($produkTypeData);
$jsFeatures = json_encode($featuresData);
?>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <h2 class="text-2xl font-semibold text-gray-700 mb-6">Edit Produk: <?= htmlspecialchars($old_data['judul'] ?? 'Tidak Ditemukan') ?></h2>
                ...
                <form id="product-form" action="<?= (BASE_URL ?? '.') . '/admin-lab/products/' . urlencode($product['id'] ?? '') ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-6 p-4 border border-gray-200 rounded-lg">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    name="judul"
                                    required
                                    value="<?= htmlspecialchars($old_data['judul'] ?? '', ENT_QUOTES) ?>"
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows="6"
                                    required
                                    class="block w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-3 py-2"><?= htmlspecialchars($old_data['deskripsi'] ?? '', ENT_QUOTES) ?></textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    id="produk_url"
                                    name="produk_url"
                                    value="<?= htmlspecialchars($old_data['produk_url'] ?? '', ENT_QUOTES) ?>"
                                    class="block w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-3 py-2"
                        ...
                        <div class="space-y-6 p-4 border border-gray-200 rounded-lg">
                            ...
                            <label for="image_file" class="block text-sm font-medium text-gray-700">
                                Unggah Gambar Utama Baru (Kosongkan untuk mempertahankan yang lama)
                            </label>
                            ...
                            <?php if (!empty($current_image_url)) : ?>
                                <div class="mt-2 mb-3">
                                    ...
                                    <img src="<?= BASE_URL . '/' . htmlspecialchars($current_image_url) ?>" alt="Gambar saat ini" class="h-32 w-auto rounded-md border border-gray-300">
                                </div>
                            <?php endif; ?>
                            
                            <div class="mt-1 flex items-center space-x-4">
                                <img id="image-preview" class="max-h-32 w-auto rounded-md object-cover hidden border border-gray-300" alt="Pratinjau Gambar Baru">
                                <div>
                                    <input id="image_file_input" name="image_file" type="file" 
                                             ...
                                             accept="image/*">
                                    <p class="text-xs text-gray-500 mt-1">PNG, JPG, GIF hingga 5MB (Opsional).</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <input type="hidden" name="published_at" value="<?= htmlspecialchars($old_data['published_at'] ?? date('Y-m-d H:i:s')) ?>">
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createTypeTag(tag) { 
        ...
        deleteBtn.addEventListener('click', function() {
            const index = produkType.findIndex(t => t === tag); 
            if (index > -1) {
                 produkType.splice(index, 1);
            }
            renderProdukType();
        });
    ...
    function renderProdukType() {
        ...
        produkType.forEach((tag) => {
            typeContainer.appendChild(createTypeTag(tag));
        });
    }
    ...
    function createFeatureElement(data, index) {
        ...
        const html = `
            <div class="md:col-span-3">
                ...
                <input value="${featureTitle}" type="text" 
            ...
            <div class="md:col-span-1 flex items-end justify-end">
                <button type="button" class="text-xs text-rose-500 hover:text-rose-700 delete-feature-btn" data-feature-index="${index}"><i class="fa fa-trash mr-1"></i>Hapus</button>
            </div>
        `;
        ...
        deleteBtn.addEventListener('click', function() {
            const indexToDelete = parseInt(this.dataset.featureIndex); 
        ...
        inputElement.addEventListener('input', function() {
            const dataIndex = parseInt(wrapper.dataset.index);
            updateFeatureData(dataIndex, this.value);
        });
    ...
    function syncFeaturesBeforeSubmit() {
        const existingHiddenInputs = form.querySelectorAll('input[name^="features["]');
        existingHiddenInputs.forEach(input => input.remove());

        features.forEach((feature, index) => {
            if (feature.judul && feature.judul.trim() !== '') {
                const input = document.createElement('input');
                input.type = 'hidden';
                input.name = `features[${index}][judul]`; 
                input.value = feature.judul;
