<?php 
$pageTitle = 'Tambah Course Baru';
$activeMenu = 'course'; // Pastikan sidebar menu highlight logika-nya sesuai
?>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <h2 class="text-2xl font-semibold text-gray-700 mb-6">Detail Course Baru</h2>

                <!-- Action URL sesuaikan dengan routing kamu -->
                <form action="<?= BASE_URL ?? '.' ?>/admin-lab/course" method="POST">
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="icon_name" class="block text-sm font-medium text-gray-700">
                                    Nama Icon/File <span class="text-gray-400 text-xs">(contoh: brain-icon)</span>
                                </label>
                                <input type="text" id="icon_name" name="icon_name" 
                                       value="<?= htmlspecialchars($old['icon_name'] ?? '') ?>"
                                       class="block w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" id="total_sessions" name="total_sessions" required
                                       value="<?= htmlspecialchars($old['total_sessions'] ?? '') ?>"
                                       class="block w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <?php if (isset($errors['total_sessions'])): ?>
                                    <p class="text-red-500 text-xs mt-1"><?= $errors['total_sessions'] ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="level" name="level" required
                                ...
                                <option value="Beginner" <?= (isset($old['level']) && $old['level'] == 'Beginner') ? 'selected' : '' ?>>Beginner</option>
                                <option value="Intermediate" <?= (isset($old['level']) && $old['level'] == 'Intermediate') ? 'selected' : '' ?>>Intermediate</option>
                                <option value="Advanced" <?= (isset($old['level']) && $old['level'] == 'Advanced') ? 'selected' : '' ?>>Advanced</option>
                            </select>
                            <?php if (isset($errors['level'])): ?>
                                <p class="text-red-500 text-xs mt-1"><?= $errors['level'] ?></p>
                            <?php endif; ?>
                        </div>
                        ...
                        <div>
                            <label for="action_url" class="block text-sm font-medium text-gray-700">
                                Link Pendaftaran (Action URL)
                            </label>
                            <input type="text" id="action_url" name="action_url" placeholder="/register/course-name"
                                   value="<?= htmlspecialchars($old['action_url'] ?? '') ?>"
                                   class="block w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
