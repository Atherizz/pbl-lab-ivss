...
// Ambil Role User saat ini untuk logika tampilan
// Pastikan session sudah start di controller atau index.php utama
                                ...
                                <div class="px-4 py-3 bg-gray-50 border-t border-gray-200 flex justify-between items-center">
                                    ...
                                    <?php if ($userRole === 'admin_lab') : ?>
                                        <div class="flex items-center space-x-3">
                                            <a href="<?= $baseUrl ?>/admin-lab/dataset/<?= $row['id'] ?>/edit" 
                                            ...
                                            <form action="<?= $baseUrl ?>/admin-lab/dataset/<?= $row['id'] ?>/delete" method="POST" 
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="overflow-hidden border border-gray-200 rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="modal-links-body" class="bg-white divide-y divide-gray-200"></tbody>
                    </table>
                </div>
            </div>
            <div class="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                <button onclick="closeUrlModal()" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors focus:outline-none">Tutup</button>
            </div>
        </div>
    </div>
</div>
...
<script>
            ...
            urls.forEach((url, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                        <a href="${url}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-800 inline-flex items-center transition-colors">Buka <svg class="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg></a>
                    </td>`;
                tbody.appendChild(row);
            });
        } else {
            tbody.innerHTML = '<tr><td colspan="3" class="px-6 py-8 text-center text-gray-500 text-sm bg-gray-50">Tidak ada link tersedia.</td></tr>';
        }
    ...
    document.addEventListener('keydown', function(event) { if (event.key === "Escape") closeUrlModal(); });
</script>